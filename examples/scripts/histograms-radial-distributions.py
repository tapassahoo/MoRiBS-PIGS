...
plt.hist(data_plot, bins='auto', density=True, color='green', alpha=0.75, edgecolor='black', label=label_str)
plt.ylabel(r'$\mathrm{\rho(r)(\AA^{-1})}$',labelpad=10)
...
#plt.text(xmin+0.5*(xmax-xmin),ymin+0.6*(ymax-ymin),r'$\mathrm{Spectroscopic \ geometry \ of \  monomer}$')
#plt.text(xmin+0.5*(xmax-xmin),ymin+0.6*(ymax-ymin),r'$\mathrm{Averaged \ geometry \ of \  monomer}$')
plt.text(xmin+0.5*(xmax-xmin),ymin+0.6*(ymax-ymin),r'$\mathrm{qTIP4P/Fw \ geometry \ of \  monomer}$')

plt.xlim(2.3905,3.409)
#plt.ylim(0.0,4.51)
plt.subplots_adjust(top=0.99,bottom=0.13,left=0.12,right=0.99,hspace=0.0,wspace=0.0)
plt.legend(numpoints=1,loc=('upper right'))

FilePlotDensity="/home/tapas/ResultsOfPIGS"+dir_read[31:-1]+"-histogram-r-preskip"+str(preskip)+"-postskip"+str(postskip)+".png"
print(FilePlotDensity)
plt.savefig(FilePlotDensity, dpi=50)
#plt.savefig(FilePlotDensity, dpi=50, format='png')
plt.show()

#python histograms-radial-distributions.py /scratch/tapas/nonlinear-rotors/PIGS-qTIP4P-paramas-matt-TransAndRotDOFs-Rpt6.0Angstrom-beta0.05Kinv-Blocks20000-Passes200-System2-p-H2O-e0vsbeads121/ 2 0

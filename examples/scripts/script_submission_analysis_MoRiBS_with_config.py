...
parser.add_argument("--RIC", action="store_true", help="It helps to read out#.xyz files.")
...
if (NameOfServer == "graham"):
	...
	dir_output_read = "/scratch/"+user_name+"/"+out_dir_read
else:
	dir_output      = "/work/"+user_name+"/"+out_dir            
	dir_output_read = "/work/"+user_name+"/"+out_dir_read
...
if (args.RIC == True):
	readConfig = True
	config=[numbblocks - 200*x for x in range(maxloop)]
else:
	readConfig = False
	config=[]
...
#==================================Generating files for submission================#
	file1_name1 = support.GetFileNameSubmission(TypeCal, molecule_rot, TransMove, RotMove, Rpt, gfact, dipolemoment, parameterName, parameter, numbblocks, numbpass, numbmolecules1, molecule, ENT_TYPE, particleA, extra_file_name, crystal)
	if (readConfig == True):
		file2_name = support.GetFileNameSubmission(TypeCal, molecule_rot, TransMove, RotMove, Rpt, gfact, dipolemoment, parameterName, parameter, numbblocks, numbpass, numbmolecules1, molecule, "SWAPTOUNSWAP", 1, "", crystal)
	for iConfig in config:
		if (readConfig == True):
			file1_name = file1_name1+"-Config-"+str(iConfig)+"-"
		else:
			file1_name = file1_name1
#===============================================================================
#                                                                              |
		...
		for i in list_nb:
			...
			if (TypeCal == 'PIMC'):
				...
				if status   == "submission":
					...
					if (readConfig == True):
						file_read = dir_output_read+file2_name+str(numbbeads)+"/results/output"+str(iConfig)+".xyz"
						if (os.path.exists(file_read) == True):
							file_read_cp = dir_run_input_pimc+"/toberead-P"+str(numbbeads)+".xyz"
							call(["cp", file_read, file_read_cp])
							call(["sed", "-i", "1,2d", file_read_cp])
						else:

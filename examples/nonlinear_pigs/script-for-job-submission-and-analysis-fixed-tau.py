...
def jackknife(func):
	...
	n             = len(func)
	idx           = np.arange(n)
	return np.sum(func[idx!=i] for i in range(n))/(float(n)-1.)

def jackknife_var(func):
	"""Jackknife estiamte of the variance of the estimator func."""
	n             = len(func)
	idx           = np.arange(n)
	j_est         = jackknife(func)
	mean          = np.sum(j_est)/float(n)
	xx            = np.multiply(j_est, j_est)
	meansq        = np.sum(xx)/float(n)
	return sqrt((meansq-mean*mean)/n)
...
for i in range(1,nrange+1):
	...
	#		print jackknife_var(col_tot),mean_sq_tot-mean_tot*mean_tot

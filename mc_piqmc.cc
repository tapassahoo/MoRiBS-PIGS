...
void MCMolecularMovePIGS(int type)
{
	int numb = MCAtom[type].numb;  
...
void MCRotLinStepSwap(int it1,int offset,int gatom,int type,double step,double rand1,double rand2,double rand3,double &MCRotChunkTot,double &MCRotChunkAcp, string Distribution)
{
	...
	double EulangOld[3], EulangNew[3];
	EulangOld[PHI] = phi;
	EulangOld[CTH] = acos(cost);
	EulangOld[CHI] = 0.0;
...
//
	...
	if (Accepted)
	{
		...
		for (int id=0;id<NDIM;id++) MCCosine[id][t1] = newcoords[id][t1];
	}
}
...
{
#ifdef DEBUG_PIMC
	const char *_proc_=__func__;    //  MCRotations3D()
	if (type != IMTYPE)
	nrerror(_proc_,"Wrong impurity type");
	...
	for (int itrot=0;itrot<NumbRotTimes;itrot=itrot+2) {
		for(int atom0=0;atom0<MCAtom[type].numb;atom0++) {
			...
			if (ENT_SIM) {	
				if (ENT_ENSMBL==EXTENDED_ENSMBL) MCRot3DstepSwap(itrot,offset,gatom,type,step,rand1,rand2,rand3,rand4,IROTSYM,NFOLD_ROT,MCRotChunkTot,MCRotChunkAcp,Distribution);
				if (ENT_ENSMBL==BROKENPATH) MCRot3DstepBrokenPath(itrot,offset,gatom,type,step,rand1,rand2,rand3,rand4,IROTSYM,NFOLD_ROT,MCRotChunkTot,MCRotChunkAcp);
			}
		}
	}
...
void MCRot3DstepSwap(int it1, int offset, int gatom, int type, double step, double rand1, double rand2, double rand3, double rand4, int IROTSYM, int NFOLD_ROT, double &MCRotChunkTot, double &MCRotChunkAcp, string Distribution) //Original function is MCRot3Dstep()
{
	int it0 = (it1 - 1);
	int it2 = (it1 + 1);
...
// Rotors partitioning
	...
	if (RotDenType == 0) 
	{
		...
		if (((it1==beadM) || (it1==beadMminus1)) && ((gatom >= particleA1Min) && (gatom <= particleA2Max)))
		{
			if (ENT_ALGR == RATIOTRICK)
			{
				if (((Distribution == "unSwap") && (RefAtom >= 2)) || (Distribution == "Swap"))
				{
					dens_old = GetDensity3DENT(Distribution, gatom, type, particleA1Min, particleA1Max, particleA2Min, particleA2Max, it0, it1, it2, Eulan0, Eulan1, Eulan2);
				}
			}
			else
			{
	...
	if(RotDenType == 0)
	{
		...
		if (((it1 == beadM) || (it1 == beadMminus1)) && ((gatom >= particleA1Min) && (gatom <= particleA2Max)))
		{
			if (ENT_ALGR == RATIOTRICK)
			{
				if (((Distribution == "unSwap") && (RefAtom >= 2)) || (Distribution == "Swap"))
				{
					dens_new = GetDensity3DENT(Distribution, gatom, type, particleA1Min, particleA1Max, particleA2Min, particleA2Max, it0, it1, it2, Eulan0, Eulan1, Eulan2);
				}
			}
			else
			{
	...
	double pot_new  = 0.0;
	for (int it=itr0;it<itr1;it++) {
		pot_new  += (PotRotE3DSwap(iRefAtom,gatom,Eulan1,it,Distribution));
	}
...
double GetDensity3DENT(string Distribution, int gatom, int type, int particleA1Min, int particleA1Max, int particleA2Min, int particleA2Max, int it0, int it1, int it2, double *Eulan0, double *Eulan1, double *Eulan2)
{
	double rho = 0.0;
	double erot = 0.0;
	double esq  = 0.0;
...
double PotEnergyPIGS(int atom0, double **pos)   
{
	int type0=MCType[atom0];
	string stype0 = MCAtom[type0].type;
   	...
   	{ 
		...
		for (int id = 0; id < NDIM; id++)
        {
            spot3d += 0.5*pos[id][t0]*pos[id][t0];
        }
		double weight = 1.0;
        spot_beads   += weight*spot3d;
	}
...
#pragma omp parallel for reduction(+: MCRotChunkTot,MCRotChunkAcp) private(rand1,rand2)
	for (int itrot=0;itrot<NumbRotTimes;itrot+=2)
	{
		for(int atom0=0;atom0<MCAtom[type].numb;atom0++)
		{
...
void MCRotLinStepIndex(int it1,int offset,int gatom,int type,double step,double rand1,double rand2,double &MCRotChunkTot,double &MCRotChunkAcp)
{
	...
	double cost = cost_indices[MCIndices[t1]];
	double phi  = phi_indices[MCIndices[t1]];
	double EulangOld[NDIM], EulangNew[NDIM];
	EulangOld[PHI] = phi;
	EulangOld[CTH] = acos(cost);
	EulangOld[CHI] = 0.0;
...
// the new density 
	int index1 = GetRotDensIndex(MCIndices[t0], MCIndices[t2], type, rand1);
	cost = cost_indices[index1];
	phi  = phi_indices[index1];
   ...
   double dens_new;
	...
	bool Accepted = false;
	if (rd>1.0)         Accepted = true;
	else if (rd>rand2) Accepted = true;

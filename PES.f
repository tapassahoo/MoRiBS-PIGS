...
c     Symmetry of the C60 cage is Ih.
c     input: coordinates of the center of mass of HF molecule 
c            are defined by R, AngleL(1)=th, AngleL(2)=ph;
c            rotation of HF in the centre of mass
c            coordinate system is defined by AngleJ(1)=thp
c            and AngleJ(2)=php;
c            R is in Angstrom and Angles are in radians
c            It should be noted, that the expansion coefficients
c            used here are those from the Table I of the paper
c            devided by factor=sqrt((2*p1(i)+1)*(2*p2(i)+1))
c     output: energy in kcal/mol  
c     This potential is based on the DF-LMP2/cc-pVTZ calculations
c     (with the BSSE correction) by Y. N. Kalugina nd P.-N. Roy;
c     University of Waterloo, Chemistry Department
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     $     xp1,xp2,xp,xr1,xr2,xr,thrj,pi
      ...
      do r1=-p1,p1
         do r2=-p2,p2
            ...
            if (thrj(xp1,xp2,xp,xr1,xr2,-xr).eq.0.d0) cycle

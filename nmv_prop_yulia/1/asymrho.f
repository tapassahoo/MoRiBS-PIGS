...
c yulia      call calfac(fact,maxfac)
...
cyulia      write(6,'(''CLASSICAL: Z='',F12.6,'' E='',F12.6,'' CM-1'',
c     +       '' E='',F12.6,'' K'','' Cv='',F12.6,'' Kb'')')
c     +       zparcl,eavrcl,eavrcl/boltz,cvcla
...
c ... classical values
      zparcl=zparev+zparod
      eavrcl=eavrcl/zparcl
c yulia (I have added jmax value before classical value of Z)
      write(6,'(''maxj =  '',I4,''  CLASSICAL: Z='',F12.6,'' E='',F12.6,
     +       '' CM-1'',
     +       '' E='',F12.6,'' K'')')
     +       jmax,zparcl,eavrcl,eavrcl/boltz
...
c------------------------------------------------------------------
 ...
 100  H=H-F*G
  ...
  190 CONTINUE
      ...
      END
c------------------------------------------------------------------
      double precision function wigd(j,m,k,theta)
c yulia,maxfac,fact)
...
c ... It takes Eq. 3.57 of Zare, 1988.
      ...
      pre1=0.5q+00*(lgamma(dfloat(j+k+1))+lgamma(dfloat(j-k+1))+
     +     lgamma(dfloat(j+m+1))+lgamma(dfloat(j-m+1)))


c yulia      pre10=sqrt(fact(j+k))*sqrt(fact(j-k))*sqrt(fact(j+m))*
c     +     sqrt(fact(j-m))
...
c ... summation over nu
      do nu=nulow,nuup
        ...
        denorm=lgamma(dfloat(j-m-nu+1))+
     +         lgamma(dfloat(j+k-nu+1))+
     +         lgamma(dfloat(nu+m-k+1))+lgamma(dfloat(nu+1))

c yulia        denorm0=fact(j-m-nu)*fact(j+k-nu)*fact(nu+m-k)*fact(nu)
c     +          *(-1)**(nu)

        pre2=pre1-denorm
        pre2=((-1)**(nu)*exp((pre2)))

c yulia        pre20=pre10/denorm0

c        write(33,*)  pre2,pre20

        cosfac=cos(thehlf)
        sinfac=-sin(thehlf)
        cosfac=cosfac**dfloat(2*j+k-m-2*nu)
        sinfac=sinfac**dfloat(m-k+2*nu)

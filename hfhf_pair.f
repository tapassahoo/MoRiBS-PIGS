...
c      data d/0.00d0,0.0d0,0.45d0/ !in ea0 pbe0/def2-tzvpp 
...
c      call matpre(Eulang_1, rotmat_1)
  

c     prepare rotational matrix for water2
c     and coordinates of atoms in space sixed frame
c      call matpre(Eulang_2, rotmat_2)
...
c      do i=1,3
c       do j=1,3
c         d1(i)=d1(i)+rotmat_1(i,j)*d(j)
c         d2(i)=d2(i)+rotmat_2(i,j)*d(j)
c       enddo
c      enddo
cc
      do i=1,3
        d1(i)=Eulang_1(i)*d(3)/2.54d0 !in au
        d2(i)=Eulang_2(i)*d(3)/2.54d0 !in au
      enddo
...
c      write(21,*)R,Eulang_1(1:3),Eulang_2(1:3),E_12

...
//#pragma omp parallel for reduction(+: MCRotChunkTot,MCRotChunkAcp) private(rand1,rand2,rand3)
	...
	{
		for (int itrot=0;itrot<NumbRotTimes;itrot += 1)
		{
...
//     phi  = phi  + M_PI;
	...
	if (abs(cost) > 2.0) 
	{
        cout<<"Upper or lower limit of cost is exceeded " << cost<<endl;
		exit(0);
	}
#endif
...
//#ifdef ENTANGLEMENT
        if (gatom > ((NumbAtoms/4) - 1) && gatom < (3*NumbAtoms/4))
        {
        	if ((t1 == 0) || (t1 == (NumbRotTimes - 1)) || (t1 == ((NumbRotTimes - 1)/2)) || (t1 == (((NumbRotTimes - 1)/2) - 1)))
        	{
            	if ((t1 == (((NumbRotTimes - 1)/2) - 1)) || (t1 == (NumbRotTimes - 1)))
            	{
                	dens_old = SRotDens(p0,type);
            	}
            	if ((t1 == 0) ||  (t1 == ((NumbRotTimes - 1)/2)))
            	{
                	dens_old = SRotDens(p1,type);
            	}
			}
...
//#error Compiler needs ENTANGLEMENT flags
//#endif
//#else
//        dens_old = SRotDens(p0,type)*SRotDens(p1,type);
//#endif
